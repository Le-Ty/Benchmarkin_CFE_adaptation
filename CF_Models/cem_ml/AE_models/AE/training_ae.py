...
def __main__():
    ...
    if data_name == 'adult_full.csv':
    ...
    elif data_name == 'compas-scores.csv':
        
        data = pd.read_csv(data_path + data_name)
        columns = data.columns
        continuous_features = ['age', 'juv_fel_count', 'decile_score', 'juv_misd_count', 'juv_other_count',
                               'priors_count', 'days_b_screening_arrest', 'c_days_from_compas', 'c_charge_degree',
                               'r_charge_degree', 'v_decile_score', 'decile_score_2', 'c_jail_time', 'r_jail_time']
        ...
        if not small_model:
            ...
            # training and saving autoencoder model
            dim_input = xtrain.shape[1]
            model = model_ae.Train_AE(dim_input, 20, 10, 7, data_name)
            model.Build_Train_Save_Model(xtrain, ytrain, xtest, ytest)
