...
from CF_Models.clue_ml.src.utils import Ln_distance
from CF_Models.clue_ml.src.probability import decompose_entropy_cat, decompose_std_gauss
...
def vae_gradient_search(instance, model, VAE, lr=0.50, prediction_similarity_weight=0,
	...
	CLUE_explainer = CLUE(VAE, model, instance, uncertainty_weight=uncertainty_weight,
	                      aleatoric_weight=aleatoric_weight, epistemic_weight=epistemic_weight,
	                      prior_weight=prior_weight, distance_weight=distance_weight,
	                      latent_L2_weight=latent_L2_weight, prediction_similarity_weight=prediction_similarity_weight,
	                      lr=lr, desired_preds=desired_preds, cond_mask=None, distance_metric=dist,
	                      norm_MNIST=False, prob_BNN=False, flatten_BNN=False, regression=False, cuda=False)
...
class CLUE(BaseNet):
	...
	def __init__(self, VAE, BNN, original_x, uncertainty_weight, aleatoric_weight, epistemic_weight, prior_weight,
	             distance_weight, latent_L2_weight, prediction_similarity_weight, lr, desired_preds=None,
	             cond_mask=None,
	             distance_metric=None, z_init=None, norm_MNIST=False, flatten_BNN=False,
	...
	def uncertainty_from_z(self):
		...
		if self.prob_BNN:
			...
			if self.regression:
				...
				probs = self.BNN.prob_predict(to_BNN)  # changed to prob_predict (name of our pytorch predict proba method)
	...
	def get_objective(self, x, total_uncertainty, aleatoric_uncertainty, epistemic_uncertainty, preds):
		...
		if self.VAE is not None and self.cond_mask is None and self.prior_weight > 0:
			try:
				...
				prior_loglike = self.VAEAC.get_prior(self.original_x, self.cond_mask, flatten=False).log_prob(self.z).sum(
					dim=1)
	...
	@staticmethod
	def apply_stopvec(it_mask, uncertainty_vec, epistemic_vec, aleatoric_vec, dist_vec, cost_vec, z_vec, x_vec,
	                  n_early_stop):
	...
	def batch_optimise(cls, VAE, BNN, original_x, uncertainty_weight, aleatoric_weight, epistemic_weight, prior_weight,
		...
		for train_idx in idx_iterator:
			...
			CLUE_runner = cls(VAE, BNN, original_x[train_idx], uncertainty_weight, aleatoric_weight, epistemic_weight,
			                  prior_weight, distance_weight,
			                  latent_L2_weight, prediction_similarity_weight, lr, cond_mask=cond_mask,
...
class conditional_CLUE(CLUE):
	...
	def __init__(self, VAEAC, BNN, original_x, uncertainty_weight, aleatoric_weight, epistemic_weight, prior_weight,
	             distance_weight,
	             lr, cond_mask=None, distance_metric=None, z_init=None, norm_MNIST=False, flatten_BNN=False,
	             regression=False, cuda=True):
		
		super(conditional_CLUE, self).__init__(VAEAC, BNN, original_x, uncertainty_weight, aleatoric_weight,
		                                       epistemic_weight,
		                                       prior_weight, distance_weight,
	...
	def uncertainty_from_z(self):
		...
		if self.regression:
			...
			probs = self.BNN.sample_predict(to_BNN, Nsamples=0, grad=False)  # Martin: changed to False
			total_uncertainty, aleatoric_uncertainty, epistemic_uncertainty = decompose_entropy_cat(probs)
