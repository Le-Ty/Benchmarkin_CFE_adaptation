...
def graph_search(data, index, keys_mutable, keys_immutable, continuous_cols, binary_cols, model,
	...
	# We implemented the radius graph and the knn graph methods. The density graph will be added in the future.
	...
	:param mode: str; either 'knn', 'kde' or 'epsilon'
	...
	chosen_indeces = np.random.choice(np.arange(0, index).tolist() + np.arange(index+1, data.values.shape[0]).tolist(),
									  replace=False, size=number_samples)
	chosen_indeces = [index] + chosen_indeces.tolist()  # make sure sample under consideration included
	...
	if mode == 'knn':
		...
		n_neighbors_list = [n_neighbors - neihboorhood_size, n_neighbors, n_neighbors + neihboorhood_size]
		...
		for n in n_neighbors_list:
			...
			distances = csgraph.dijkstra(csgraph=graph, directed=False, indices=index, return_predecessors=False)
			...
			candidate_min_distances = [min_distance, min_distance+1, min_distance+2, min_distance+3]
			...
			indeces_counterfactuals = np.intersect1d(np.array(y_positive_indeces), np.array(min_distance_indeces))
		...
		radius_neighbors_list = [radius-radius_neighboorhood_size, radius, radius+radius_neighboorhood_size]
		candidate_counterfactuals_star = []
	...
	if candidate_counterfactual_star.size == 0:
		candidate_counterfactual_star = np.empty(data.values.shape[1], )
