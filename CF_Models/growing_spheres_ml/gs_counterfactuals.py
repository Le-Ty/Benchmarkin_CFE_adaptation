...
def growing_spheres_search(instance, keys_mutable, keys_immutable, continuous_cols, binary_cols, model,
	...
	instance_mutable_replicated_continuous = np.repeat(instance[keys_mutable_continuous].values.reshape(1, -1),
													   n_search_samples, axis=0)
	instance_mutable_replicated_binary = np.repeat(instance[keys_mutable_binary].values.reshape(1, -1),
												   n_search_samples, axis=0)
	...
	while True:
		...
		candidate_counterfactuals_binary = np.random.binomial(n=1, p=0.5,
															  size=n_search_samples*len(keys_mutable_binary)).reshape(n_search_samples,-1)
		
		# make sure inputs are in correct order
		candidate_counterfactuals = pd.DataFrame(np.c_[instance_immutable_replicated,
												 candidate_counterfactuals_continuous,
												 candidate_counterfactuals_binary])
		candidate_counterfactuals.columns = keys_immutable + keys_mutable_continuous + keys_mutable_binary
